...
export function GridPagination({ table }: GridPaginationProps) {
  const { pageIndex, pageSize } = table.getState().pagination
  const pageCount = table.getPageCount()
  const canPrev = table.getCanPreviousPage()
  const canNext = table.getCanNextPage()
  const from = pageIndex * pageSize + 1
  const to = Math.min((pageIndex + 1) * pageSize, table.getFilteredRowModel().rows.length)
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-3 py-2 border-t bg-card text-sm flex-wrap">
      ...
      <div className="flex items-center gap-1">
        <Button variant="outline" size="icon-sm" onClick={() => table.setPageIndex(0)} disabled={!canPrev} className="size-7">
          <ChevronFirst className="size-3.5" />
        </Button>
        <Button variant="outline" size="icon-sm" onClick={() => table.previousPage()} disabled={!canPrev} className="size-7">
        ...
        <div className="flex items-center gap-1.5 px-2">
          ...
          />
          <span className="text-xs text-muted-foreground">of {pageCount.toLocaleString()}</span>
        </div>

        <Button variant="outline" size="icon-sm" onClick={() => table.nextPage()} disabled={!canNext} className="size-7">
          <ChevronRight className="size-3.5" />
        </Button>
        <Button variant="outline" size="icon-sm" onClick={() => table.setPageIndex(pageCount - 1)} disabled={!canNext} className="size-7">
